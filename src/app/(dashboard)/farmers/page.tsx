...
export default async function FarmersPage({ searchParams }: { searchParams: { search?: string } }) {
  const supabase = createClient();
  const search = searchParams.search?.trim();

  let query = supabase.from("farmers").select("id, farmer_code, name, phone, address, status").order("farmer_code");
  if (search) {
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm md:block dark:border-slate-700 dark:bg-slate-800">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {rows.map((farmer) => (
              <tr key={farmer.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
