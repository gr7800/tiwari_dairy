...
import { createSupply, type SupplyActionState } from "@/lib/actions/supplies";
...
export function SupplyForm({
  ...
  const [state, formAction] = useFormState(createSupply, initialState);
  useActionToast(state, "Supply entry saved");
  ...
  return (
    <form action={formAction} className="max-w-2xl space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <h2 className="font-heading text-lg font-semibold text-slate-900 dark:text-slate-100">Add Milk Supply</h2>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FieldGroup label="Date">
          <Input type="date" name="supplyDate" value={date} onChange={(e) => setDate(e.target.value)} required />
        </FieldGroup>
